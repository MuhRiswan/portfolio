  ...
  return (
    <section className="py-20 bg-surface-dark/10 px-6" id="projects">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          <div className="max-w-2xl">
            ...
            <p className="text-lg text-slate-400 dark:text-slate-400 leading-relaxed">
              Here are some of the standout projects I have built—both from my
              professional work and personal initiatives. Each project reflects
              my expertise, dedication, and passion for web development.
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center bg-slate-100 dark:bg-white/5 p-1 rounded-2xl w-fit">
            {CATEGORIES.map((category) => (
              <button
                ...
                className={`px-6 py-2.5 text-sm font-bold rounded-xl transition-all duration-300 ${selectedCategory === category ? "bg-white dark:bg-primary text-slate-900 dark:text-white shadow-lg" : "text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
              >
                {category}
              </button>
            ))}
        ...
        <div
          className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 transition-opacity duration-300 ${searchQuery !== deferredSearchQuery ? "opacity-50" : "opacity-100"}`}
        >
